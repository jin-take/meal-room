...
import { ArrowDown, ArrowUp, ArrowLeft, CalendarDays, ChefHat, Check, ChevronRight, CloudUpload, Copy, ExternalLink, Home, LogOut, Plus, Search, ShoppingBasket, Trash2, UserRound, Users, X } from 'lucide-react';
...
function Dashboard({data,onTab}:{data:RoomData;onTab:(t:Tab)=>void}) {
  ...
  return <section className="stack"><div className="hero-card"><span className="eyebrow">TODAY</span><h2>{format(new Date(),'M月d日 EEEE',{locale:ja})}</h2><p>{names.length?names.join('・'):'今日の献立はまだ空いています。'}</p><button className="secondary" onClick={()=>onTab('calendar')}>献立を決める</button></div><div className="stats-grid"><button className="stat-card" onClick={()=>onTab('recipes')}><ChefHat/><b>{data.recipes.length}</b><span>登録したレシピ</span></button><button className="stat-card" onClick={()=>onTab('shopping')}><ShoppingBasket/><b>{data.shoppingItems.filter(i=>!i.checked).length}</b><span>買うもの</span></button><button className="stat-card" onClick={()=>onTab('room')}><Users/><b>{data.members.length}</b><span>メンバー</span></button></div><div className="section-head"><div><span className="eyebrow">THIS WEEK</span><h2>今週の献立</h2></div></div><WeekPreview data={data}/></section>
}
...
function RecipePickerModal({recipes,currentRecipeId,date,slot,onSelect,onClose}:{recipes:Recipe[];currentRecipeId:string;date:string;slot:MealSlot;onSelect:(recipeId:string)=>void;onClose:()=>void}) {
 ...
 return <div className="modal-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}>
  <div className="recipe-picker-modal" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId}>
   <div className="modal-head"><div><span className="eyebrow">{format(parseISO(date),'M月d日（E）',{locale:ja})}・{slotLabels[slot]}</span><h3 id={titleId}>レシピを選ぶ</h3></div><button type="button" className="modal-close" onClick={onClose} aria-label="閉じる"><X size={20}/></button></div>
   <div className="recipe-search"><Search size={18}/><input ref={inputRef} type="search" value={query} onChange={event=>setQuery(event.target.value)} placeholder="レシピ名・カテゴリ・食材で検索" aria-label="レシピを検索"/>{query&&<button type="button" onClick={()=>{setQuery('');inputRef.current?.focus()}} aria-label="検索をクリア"><X size={17}/></button>}</div>
   <div className="recipe-options">
    <button type="button" className={!currentRecipeId?'recipe-option selected':'recipe-option'} onClick={()=>onSelect('')}><div><strong>未定</strong><span>この枠のレシピを設定しない</span></div>{!currentRecipeId&&<Check size={19}/>}</button>
    ...
    {filtered.length===0&&<div className="recipe-search-empty"><Search size={24}/><p>「{query.trim()}」に一致するレシピがありません。</p></div>}
   </div>
  </div>
 </div>
}
...
function Shopping({data,commit}:{data:RoomData;commit:(d:RoomData)=>void}) {
 ...
 const generate=()=>{if(!from||!to||rangeError)return;const ids=new Set(data.mealPlans.filter(p=>p.date>=from&&p.date<=to).map(p=>p.recipeId));const ingredients=[...new Set(data.recipes.filter(r=>ids.has(r.id)).flatMap(r=>r.ingredients).map(normalizeIngredient).filter(Boolean))];const checked=new Map(data.shoppingItems.map(i=>[i.name,i.checked]));const auto=ingredients.map(name=>({id:id('shop'),name,checked:checked.get(name)||false,source:'auto' as const,rangeKey:`${from}:${to}`}));const remaining=new Map(auto.map(item=>[item.name,item]));const retained:ShoppingItem[]=data.shoppingItems.flatMap(item=>{if(item.source==='manual')return [item];const next=remaining.get(item.name);if(!next)return [];remaining.delete(item.name);return [{...next,id:item.id}]});commit({...data,shoppingItems:[...retained,...remaining.values()]})};
 const move=(sid:string,direction:number)=>{const items=[...data.shoppingItems];const index=items.findIndex(item=>item.id===sid);const target=index+direction;if(index<0||target<0||target>=items.length)return;[items[index],items[target]]=[items[target],items[index]];commit({...data,shoppingItems:items})};
 ...
 return <section className="stack"><div className="section-head"><div><span className="eyebrow">SHOPPING</span><h2>買い物リスト</h2></div></div><div className="editor-card"><h3>献立から自動作成</h3><DateRangeCalendar from={from} to={to} onChange={(nextFrom,nextTo)=>{setFrom(nextFrom);setTo(nextTo)}}/>{rangeError&&<p className="form-error">{rangeError}</p>}<button className="primary" onClick={generate} disabled={!from||!to||!!rangeError}><ShoppingBasket size={18}/>この期間の食材を集計</button></div><div className="manual-add"><input value={manual} onChange={e=>setManual(e.target.value)} onKeyDown={e=>e.key==='Enter'&&add()} placeholder="手動で追加"/><button onClick={add}><Plus/></button></div><div className="shopping-list">{data.shoppingItems.length>1&&<p className="shopping-order-hint">上下ボタンで買う順番に並べ替えられます。</p>}{data.shoppingItems.length===0&&<div className="empty">まだ買うものがありません。</div>}{data.shoppingItems.map((item,index)=><div className={`shopping-row ${item.checked?'done':''}`} key={item.id}><button className="check-button" onClick={()=>toggle(item.id)}>{item.checked&&<Check size={17}/>}</button><div><b>{item.name}</b><span>{item.source==='auto'?'献立から':'手動追加'}</span></div><div className="shopping-order-controls"><button type="button" aria-label={`${item.name}を上へ移動`} disabled={index===0} onClick={()=>move(item.id,-1)}><ArrowUp size={18}/></button><button type="button" aria-label={`${item.name}を下へ移動`} disabled={index===data.shoppingItems.length-1} onClick={()=>move(item.id,1)}><ArrowDown size={18}/></button></div><button className="trash" aria-label={`${item.name}を削除`} onClick={()=>remove(item.id)}><Trash2 size={17}/></button></div>)}</div></section>
}
