...
Room更新のたびに同じRoom IDの項目を置き換えます。招待コードの一意性検査、indexとRoom JSONのトランザクション、古い項目の自動削除は実装していません。

## バージョンと整合性

`version`は変更回数の目安として使用します。S3直接PUTでは条件付き書き込みを行わないため、競合検出や楽観ロックの保証はありません。複数端末が同時に保存すると後から完了したRoom JSONが残ります。
